...
data <- read.table("C:/Users/ldwan/Downloads/yearpredictionmsd/YearPredictionMSD.txt", sep = ",", header = F)
data <- na.omit(data)
drop <- 2:91
ds <- data[, drop]
X <- scale(ds)
...
#df_var2<-data.frame(sketch_size,#uniform_sampling=as.vector(r_subsample)*sqrt(sketch_size),
...
#combined <- p1f + p2f & theme(legend.position = "right",legend.title=element_blank())
#combined + plot_layout(guides = "collect")

#file_path<-"C:/Users/Desktop/test/plots/"
#ggsave("k1_Market_var.eps", path=file_path, width = 8, height = 4, units = "in", dpi = 300)
...
write.csv(data_val_full,"C:/Users/Desktop/test/plots/csv/k1_MSD_Coverage_val.csv")
write.csv(data_vec_full,"C:/Users/Desktop/test/plots/csv/k1_MSD_Coverage_vec.csv")
...
combined <- p1f + p2f & theme(legend.position = "none",legend.title=element_blank())
combined + plot_layout(guides = "collect")

file_path<-"C:/Users/Desktop/test/plots/"
ggsave("k1_MSD_Coverage.eps", path=file_path, width = 8, height = 6, units = "in", dpi = 300)
